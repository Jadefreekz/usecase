...
#Checks if alternate requests are declined or accepted.
...
#Step 8 : customer 202 requests a ride. Should be rejected
rideId=$(curl -s "http://localhost:8081/requestRide?custId=202&sourceLoc=40&destinationLoc=70")
if [ "$rideId" == "-1" ];
then
    echo "Ride by customer 202 rejected because it is 2nd ride"
else
    echo "Ride to customer 202 provided"
    testPassed="no"
fi

#Step 9 : Status of a cab on ride.
resp=$(curl -s "http://localhost:8081/getCabStatus?cabId=101")
if [ "$resp" != "available 200" ];
then
    echo "Invalid Status for the cab 101"
...
#Step 10 : customer 203 requests a ride. Should be accepted
rideId=$(curl -s "http://localhost:8081/requestRide?custId=203&sourceLoc=80&destinationLoc=120")
if [ "$rideId" != "-1" ];
then
...
#Step 11 : Status of a cab on ride
resp=$(curl -s "http://localhost:8081/getCabStatus?cabId=101")
if [ "$resp" != "giving-ride 80 203 120" ];
then
    echo "Invalid Status for the cab 101"
    testPassed="no"
else
...
#Step 12 : customer 204 requests a ride. Should be rejected
rideId2=$(curl -s "http://localhost:8081/requestRide?custId=204&sourceLoc=130&destinationLoc=210")
if [ "$rideId2" == "-1" ];
then
    echo "Ride by customer 204 rejected because cab in giving-ride state"
else
    echo "Ride to customer 204 is provided"
    testPassed="no"
fi
...
#Step 14 : #Status of a cab after a ride
resp=$(curl -s "http://localhost:8081/getCabStatus?cabId=101")
if [ "$resp" != "available 120" ];
then
    echo "Invalid Status for the cab 101"
    testPassed="no"
else
